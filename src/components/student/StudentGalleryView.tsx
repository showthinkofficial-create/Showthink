...
export const StudentGalleryView: React.FC<StudentGalleryViewProps> = ({ albums, media }) => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-6 rounded-3xl border border-gray-200 shadow-xs">
        ...
        {selectedAlbumId && (
          <button
            ...
            className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-800 text-xs font-bold rounded-xl transition-all shrink-0 w-fit"
          >
            ← Back to All Albums
          </button>
        )}
      ...
      {selectedAlbumId && (
        <div className="space-y-4">
          <div className="bg-white p-4 sm:p-6 rounded-3xl border border-gray-200 space-y-2">
      ...
      {activeMedia && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-3 sm:p-4">
          <button
            ...
            className="absolute top-4 right-4 sm:top-6 sm:right-6 p-2 text-white hover:text-gray-300 bg-white/10 hover:bg-white/20 rounded-full transition-all z-10 cursor-pointer"
          >
            <X className="w-6 h-6" />
          </button>
